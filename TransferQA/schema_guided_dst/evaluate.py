...
def get_dataset_as_dict(file_path_patterns):
  ...
  if isinstance(file_path_patterns, list):
    ...
    list_fp = sorted(tf.io.gfile.glob(file_path_patterns))
  for fp in list_fp:
    if PER_FRAME_OUTPUT_FILENAME in fp:
      continue
    tf.compat.v1.logging.info("Loading file: %s", fp)
    with tf.io.gfile.GFile(fp) as f:
      data = json.load(f)
...
def get_metrics(dataset_ref, dataset_hyp, service_schemas, in_domain_services):
  ...
  tf.compat.v1.logging.info("len(dataset_hyp)=%d, len(dataset_ref)=%d", len(dataset_hyp),
                  len(dataset_ref))
  ...
  for dial_id, dial_hyp in dataset_hyp.items():
    ...
    for turn_id, (turn_ref, turn_hyp) in enumerate(
      ...
      if turn_ref["utterance"] != turn_hyp["utterance"]:
        tf.compat.v1.logging.info("Ref utt: %s", turn_ref["utterance"])
        tf.compat.v1.logging.info("Hyp utt: %s", turn_hyp["utterance"])
        raise ValueError(
...
def main(_):
  ...
  all_metric_aggregate, _ = get_metrics(dataset_ref, dataset_hyp, eval_services,
                                        in_domain_services)
  tf.compat.v1.logging.info("Dialog metrics: %s", str(all_metric_aggregate[ALL_SERVICES]))

  # Write the aggregated metrics values.
  with tf.io.gfile.GFile(FLAGS.output_metric_file, "w") as f:
    json.dump(
        all_metric_aggregate,
        f,
        indent=2,
