...
def _file_based_input_fn_builder(dataset_config, input_dial_file,
  ...
  def input_fn(params):
    ...
    for key, array in schema_data_dict.items():
      schema_tensors[key] = tf.convert_to_tensor(value=np.asarray(array, np.float32))
...
class SchemaGuidedDST(object):
  ...
  def define_loss(self, features, outputs):
    ...
    num_active_intents = tf.expand_dims(
        tf.reduce_sum(input_tensor=intent_labels, axis=1), axis=1)
    ...
    intent_loss = tf.compat.v1.losses.softmax_cross_entropy(
    ...
    requested_slot_loss = tf.compat.v1.losses.sigmoid_cross_entropy(
    ...
    cat_slot_status_loss = tf.compat.v1.losses.softmax_cross_entropy(
    ...
    cat_slot_value_loss = tf.compat.v1.losses.softmax_cross_entropy(
    ...
    noncat_slot_status_loss = tf.compat.v1.losses.softmax_cross_entropy(
        tf.reshape(one_hot_labels, [-1, 3]),
    ...
    noncat_loss_weight = tf.cast(
        ...
        tf.float32)
    span_start_loss = tf.compat.v1.losses.softmax_cross_entropy(
    ...
    span_end_loss = tf.compat.v1.losses.softmax_cross_entropy(
  ...
  def define_predictions(self, features, outputs):
    ...
    predictions["intent_status"] = tf.argmax(
        input=outputs["logit_intent_status"], axis=-1)
    ...
    batch_size = tf.shape(input=end_scores)[0]
    ...
    total_scores = tf.compat.v1.where(invalid_index_mask, tf.zeros_like(total_scores),
                            total_scores)
    max_span_index = tf.argmax(
        input=tf.reshape(total_scores, [-1, max_num_slots, max_num_tokens**2]),
        axis=-1)
    span_start_index = tf.math.floordiv(max_span_index, max_num_tokens)
    span_end_index = tf.math.floormod(max_span_index, max_num_tokens)
  ...
  def _encode_utterances(self, features, is_training):
    ...
    encoded_utterance = tf.compat.v1.layers.dropout(
        encoded_utterance, rate=FLAGS.dropout_rate, training=is_training)
    encoded_tokens = tf.compat.v1.layers.dropout(
  ...
  def _get_intents(self, features):
    ...
    null_intent_embedding = tf.compat.v1.get_variable(
        "null_intent_embedding",
        shape=[1, 1, embedding_dim],
        initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))
    batch_size = tf.shape(input=intent_embeddings)[0]
    ...
    return tf.compat.v1.where(mask, logits, negative_logits)
  ...
  def _get_categorical_slot_goals(self, features):
    ...
    value_logits = tf.compat.v1.where(mask, value_logits, negative_logits)
  ...
  def _get_noncategorical_slot_goals(self, features):
    ...
    span_logits = tf.compat.v1.where(tiled_token_mask, span_logits, negative_logits)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.compat.v1.estimator.tpu.TPUEstimatorSpec(
          ...
          training_hooks=[
              tf.estimator.LoggingTensorHook(logged_tensors, every_n_iter=5)
          ])
